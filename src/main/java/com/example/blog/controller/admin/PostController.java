...
public class PostController extends BaseController {
    ...
    public String posts(Model model,
        ...
        Page<Post> posts = postService.findAll(page, new QueryCondition<>(condition, searchVo));
        for (Post temp : posts.getRecords()) {
    ...
    private void checkTags(String tagList) {
        if(tagList == null || StringUtils.isEmpty(tagList.trim())) {
            throw new MyBusinessException("请至少输入一个标签");
        }
    ...
    @ResponseBody
    public JsonResult down(@RequestParam("id") Long postId) {
        Post post = postService.get(postId);
        basicCheck(post);
        post.setPostStatus(PostStatusEnum.DOWN.getCode());
        postService.update(post);
        return JsonResult.success("操作成功");
    }
